...
<?php
	...
	$pre_ingest_staging_data = json_encode($pre_ingest_staging_data);
	$runComponent_data = json_encode($runComponent_data);
...
// Get the data
     var pre_ingest_staging_data = <?php echo $pre_ingest_staging_data; ?>;
     var runComponent_data = <?php echo $runComponent_data; ?>;
     var readyForIngest_data = <?php echo $readyForIngest_data; ?>;
 
 	//get data for pre_ingest_staging_data
	pre_ingest_staging_data.forEach(function(d) {
	...
	//get data for runComponent_data
	runComponent_data.forEach(function(d) {
		d.date = parseDate(d.date);
		d.close = +d.close;
	});
	...
	x.domain(d3.extent(pre_ingest_staging_data, function(d) { return d.date; }));
	y.domain([0, 800]);
